...
let a = 9;
let b = 13;
console.log(a > 0);      // 顯示結果：true
console.log(b > a);      // 顯示結果：true
console.log(a + b > 1);  // 顯示結果：true
let c = 51;
let d = 163;
console.log(c == d);     // 顯示結果：false
console.log(c !== d);    // 顯示結果：true
let e = 28;
let f = 45;
console.log(f >= e);     // 顯示結果：true
console.log(f != e);     // 顯示結果：true
console.log(f == e);     // 顯示結果：false

// ### 題目二：比較運算子 + 強制轉型
// 請回答每個 console.log 的值為？
let g = 8;
let h = '8';
console.log(g * h == 88);   // 顯示結果：false
console.log(g * h == 64);   // 顯示結果：true
console.log(g * h === 64);  // 顯示結果：true

let i = '9';
let j = '9';
console.log(i + j == 99);     // 顯示結果：true
console.log(i + j === '99');  // 顯示結果：true
console.log(i + j === 99);    // 顯示結果：false
...
/*
顯示結果：true
此處變數l發生自動轉型，從string '8'轉成number 8
接著k * l = 24，其大於21，故返回true
 */
...
if (coachIncome < 100000) {
  coachBonus += Math.round(coachIncome * 0.1);
} else if (coachIncome >= 100000 && coachIncome < 300000){
  coachBonus += Math.round(coachIncome * 0.15);
} else if (coachIncome >= 300000){  //此處題目文字描述不清晰，無法確認是否[含30萬]
  coachBonus += Math.round(coachIncome * 0.2);
} 
...
if (performanceData['coaches'][0]['performance'] < 50000) {
  // 請填寫第一位教練業績增長程式碼，使用 +=
  differ = 50000 - performanceData['coaches'][0]['performance']
  performanceData['coaches'][0]['performance'] += differ
}
...
if (performanceData.coaches[1].performance < 50000) {
  // 請填寫第二位教練業績增長程式碼，使用 +=
  differ = 50000 - performanceData.coaches[1].performance
  performanceData.coaches[1].performance += differ
}
...
for (let i = 0; i < exerciseRecords.length; i++) {
  if (exerciseRecords[i].duration >= 30 && exerciseRecords[i].heartRate >= 130) {
    let mondayDuration = exerciseRecords[i].duration
    totalDuration += mondayDuration;
    validDays += 1;
  };
};
...
console.log(`有效運動天數為${validDays}, 總運動時數為${totalDuration}`)
